(function (global, factory) {
	...
	(global = global || self, global.caseify = factory());
	...
	*/
	...
	var camelCase = {
	  ...
	  handler: function handler(arr) {
	    return arr.map(function (key, index) {
	      return index ? key.charAt(0).toUpperCase() + key.slice(1).toLowerCase() : key.toLowerCase();
	    }).join(this.joiner);
	  }
	};
	...
	function caseify(str, to) {
	  ...
	  if (caseTypes.has(to)) {
	    var _caseTypes$get = caseTypes.get(to),
	        joiner = _caseTypes$get.joiner,
	        handler = _caseTypes$get.handler;

	    str = str.match(/[A-Z]{2,}(?=[A-Z][a-z]+[0-9]*|\b)|[A-Z]?[a-z]+[0-9]*|[A-Z]|[0-9]+/g);
	    return handler.call(_objectSpread2({
	      joiner: joiner
	    }, options), str);
	  } else {
	    return str;
